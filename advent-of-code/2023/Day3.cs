    ...
    internal static class Day3
    {
        ...
        public static int Part1()
        {
            ...
            for (int row = 0; row < _lines.Count; row++)
            {
                ...
                {
                    // Skip if its checked
                    // Skip if its '.' and mark as checked
                    // Skip if it's number but do not mark as checked
                    if (_isChecked[row, col]) continue;

                    if (!_symbolList.Contains(_lines[row][col]))
                    {
                        if (!char.IsDigit(_lines[row][col]))
                        {
                            _isChecked[row, col] = true;
                        }
                        continue; 
                    }

                    // Symbols only now
                    sum += GetSurroundNumbers(row, col);
                }
            }
        ...
        /// <summary>
        /// Given a position of a symbol, check if there are numbers that surround this, return the sum of all of them and mark as checked
        /// </summary>
        /// <param name="row"></param>
        /// <param name="col"></param>
        /// <returns></returns>
        private static int GetSurroundNumbers(int row, int col)
        {
            // Console.WriteLine($"Getting numbers for symbol {_lines[row][col]} at {row}, {col}");
            var localSum = 0;
            for (int r = row - 1; r <= row + 1; r++)
            {
                if (r < 0 || r > _lines.Count) continue;

                for (int c = col - 1; c <= col + 1; c++)
                {
                    if (c < 0 || c > _lines.Count) continue;
                    if (_isChecked[r, c]) continue;

                    if (char.IsDigit(_lines[r][c]))
                    {
                        var thisNumber = GetNumberByPos(r, c);
                        localSum += thisNumber;
                    }
                    _isChecked[r, c] = true;
                }
        ...
        private static int CalcGearRatios(int row, int col)
        {
            var localMult = 1;
            var gearCount = 0;
            for (int r = row - 1; r <= row + 1; r++)
            {
                ...
                for (int c = col - 1; c <= col + 1; c++)
                {
                    ...
                    {
                        var thisNumber = GetNumberByPos(r, c);
                        localMult *= thisNumber;
                        gearCount++;
                    }
                    _isChecked[r, c] = true;
                }
            }
            if (gearCount == 2) return localMult;
            return 0;
        }

        /// <summary>
        /// Given a position of a number digit in the grid, return that number
        /// </summary>
        /// <param name="row"></param>
        /// <param name="col"></param>
        /// <returns></returns>
        private static int GetNumberByPos(int row, int col)
        {
            var startCol = col;
            var endCol = col;
            var thisLine = _lines[row];

            while (char.IsDigit(thisLine, startCol))
            {
                startCol--;
                if (startCol < 0) break;
            }

            while (char.IsDigit(thisLine, endCol))
            {
                endCol++;
                if (endCol > _lines[0].Length - 1) break;
            }
            
            // Mark all number digits as checked
            for (int i = startCol + 1; i < endCol; i++)
            {
                _isChecked[row, i] = true;
            }
            var thisNum = thisLine.Substring(startCol + 1, endCol - startCol - 1);
            // Console.WriteLine($"Adding number: {thisNum}");
            return int.Parse(thisNum);
        }
    }
}
